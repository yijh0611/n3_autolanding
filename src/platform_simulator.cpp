...
// // 속도제어 PD 계수 - 함수 안에서 정의 하는게 나을거 같아서 함수 안으로 옯겼음.
      ...
      if(tag_x_tmp != x_tf){ // 태그 정보가 바뀌었을때
        ...
        if(kal_is_tag_lost == 0){ // 태그 놓치지 않았을 때
          ...
          if((float)msecs_tag_time_now - (float)msecs_tag_time < 500){
            // spd_constant = (ros::Time::now().toSec() - tag_time)/0.5;
          ...
          move_right = kal_r_vel_tmp + move_right;
          move_front = kal_f_vel_tmp + move_front;
        }
        else{
        ...
        // 태그보고 고도 조절
        if(fabs(z_tf - 5.0) > 0.1){
          move_up = (5.0 - z_tf) * 0.3;
        }

        distance_data.data[5] = move_front;
        distance_data.data[6] = -1 * move_right;
        distance_pub.publish(distance_data);
