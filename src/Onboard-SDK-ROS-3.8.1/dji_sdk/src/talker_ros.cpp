...
int main(int argc, char **argv){
  ...
  // // spd(cont,0.1,0.0,0.0,0.0); // 전
  // // move(cont,2.0);

  // // spd(cont,0.0,0.0,0.0,0.0);
  // // move(cont,1.0);

  // // spd(cont,0.0,0.1,0.0,0.0); // 좌
  // // move(cont,2.0);

  // spd(cont,0.0,0.0,0.0,0.0);
  // move(cont,1.0);

  // spd(cont,0.0,0.0,0.0,0.1); // 2.0은 너무 빠른거 같다. 속도 느리게 바꿔보기 !! -> 0.1 도 가능
  // move(cont,2.0);

  // spd(cont,0.0,0.0,0.0,0.0);
  // move(cont,1.0);

  // spd(cont,0.0,0.0,0.0,-0.1);
  // move(cont,2.0);

  // spd(cont,0.0,0.0,0.0,0.0);
  // move(cont,1.0);

  // spd(cont,0.0,0.0,0.1,0.0);
  // move(cont,1.5);


  // // // 아래 주석 처리하고 비행 테스트 !!
  // // // 이 아래로 병렬처리 하는 코드 및 AprilTag 따라가는 코드.
  ...
  auto control_gimbal = []()
    ...
    */
    ...
    while(is_run_thread){
      ...
      // gimbal y 방향 회전 (tilt // 56 ~ 77(77이 정면이고, 더 안올라감))
      if(chk_y != y_tf){
        int y_threshold = 8; // 4
      ...
      ros::Duration(0.05).sleep(); // sleep for hundredth of a second
    }
	};
...
void chatterCallback2(const sensor_msgs::Imu msg2) // 이거도 안쓰는 듯.
{
...
bool takeoff_land(int task) // 이거는 아마 쓸일 없을 듯
{
...
bool monitoredTakeoff() // 이거도 아마 쓸일 없을 듯
{
  ros::Time start_time = ros::Time::now();
