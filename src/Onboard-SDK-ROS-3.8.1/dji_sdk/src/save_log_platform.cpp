...
// 시뮬레이션 관련
float tag_f_real, tag_r_real, tag_e, tag_n;
...
void cb_sim_data(const std_msgs::Float64MultiArray sim_tmp);
// gps 관련
...
int main(int argc, char **argv){
  ...
  kp_control,kd_control,spd_constant,kal_r_vel_tmp,kal_f_vel_tmp,sim_tag_f,sim_tag_r,\
  sim_tag_e,sim_tag_n";
  ...
  ros::Subscriber sub_tf = nh.subscribe("tf", 1000, callback_tf_2);
  ...
  ros::Subscriber sub_sim_data = nh.subscribe("sim_data", 1000, cb_sim_data);
  ...
  auto save_log = []()
    ...
    ros::Rate loop_rate_save(100);
      ...
      outfile << kal_f_vel_tmp << "," << tag_f_real << ",";
      outfile << tag_r_real << "," << tag_e << "," << tag_n << endl;
...
bool get_src(){
  if(!is_test){
    ...
    src_log = "/home/aims/Desktop/vid_output/" + vid_name + "_sim.csv";
  }
  else{
...
void cb_sim_data(const std_msgs::Float64MultiArray sim_tmp){
  tag_f_real = sim_tmp.data[0];
  tag_r_real = sim_tmp.data[1];
  tag_e = sim_tmp.data[2];
  tag_n = sim_tmp.data[3];
}