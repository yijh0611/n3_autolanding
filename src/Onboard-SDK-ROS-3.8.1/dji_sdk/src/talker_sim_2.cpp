...
// talker_sim_2
// 작업중 ...
// 등속 직선운동하는 플랫폼을 따라가는 코드.
// 동쪽으로 이동.
// 처음에 플랫폼이 동쪽으로 얼마나 떨어져서 출발하는지 입력 받기.
// 시속 몇 m/s으로 이동하는지 입력받기.
// 몇초동안 이동할건지 입력받기.
// Tag 까지 거리 출력해주기.
// Target 좌표 Publish 하는 코드 만들고, 로그 저장도 하게 만들면 좋을 것 같다.
...
int main(int argc, char **argv){
  ...
  auto control_drone = []()
    ...
    while(is_run_thread){
      ...
      // 계산 시작하기 전에 변수 업데이트하는게 좋을지 테스트
      float target_x_tmp = target_x;
      float target_y_tmp = target_y;
      float vo_x_tmp = vo_x;
      float vo_y_tmp = vo_y;

      // Publish Target
      target.data[0] = target_x;
      target.data[1] = target_y;
      ...
      vel_f, vel_r = enu_to_fru(vel_x, vel_y, theta);

      vel_f_prev = vel_f;
      vel_r_prev = vel_r;
      // cout << "vel_r : " << vel_r << endl;
      target.data[8] = vel_r;
      target.data[9] = theta;
      target.data[10] = theta_deg;
      target.data[11] = vo_x_tmp;

      // Get distance to target
      dist_r_prev = dist_r;
      dist_f_prev = dist_f;
      
      dist_f, dist_r = enu_to_fru(target_x_tmp - vo_x_tmp, target_y_tmp - vo_y_tmp, theta);
      
      // dist_r = target_x_tmp - vo_x_tmp; // 임시
      ...
      vel_platform_r = (dist_r - dist_r_prev) * 50; // 원래는 /0.02 였는데, 50을 곱해도 같은 결과
      vel_platform_f = (dist_f - dist_f_prev) * 50;
      // // cout << "dist_r" << endl;
      // ROS_INFO("dist_r : %f",dist_r);
      // //  dist_r << endl;
      // // cout << "dist_r_prev" << endl;
      // ROS_INFO("dist_r_prev : %f",dist_r_prev);
      // dist_r_prev << endl;
      if(vel_platform_r > 10){ // 너무 큰 값 나오면 10으로 고정 - 일단 r만 사용하므로 r만 했음.
      // 이거로 D 제어 하는데, D가 너무 크게 되어 드론 제어가 잘 안된다.
        vel_platform_r = 10;
      }
      if(vel_platform_r < -10){
        vel_platform_r = -10;
      }
      ...
      // }
      vel_platform_f_abs = vel_f + vel_platform_f;
      vel_platform_r_abs = vel_r + vel_platform_r;

      target.data[2] = vel_platform_r_abs;
      target.data[3] = vel_platform_f_abs;
      ...
      // V_platform + 거리 기반 PD제어를 해야 잘 따라갈 수 있다.
      // 플랫폼의 절대 속도 파악하는 코드
      target.data[4] = move_right;
      target.data[5] = dist_r;
      target.data[6] = vel_platform_r;

      move_right = vel_platform_r_abs + move_right;
      target.data[7] = move_right;
      target_pub.publish(target);
      ...
      // loop_rate.sleep();
      while(ros::Time::now() - control_start_time < ros::Duration(loop_count * loop_time)){

      }
    }
	};

  // // 쓰레드 3은 짐벌인데, 필요 없어서 지웠음.
