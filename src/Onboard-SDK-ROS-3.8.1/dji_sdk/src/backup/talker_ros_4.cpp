...
// talker_ros_4는 바닥에 보이는 태그를 이용해서 좀 빠르게 따라간다. (코드 미완성)
...
int main(int argc, char **argv){
  ...
  float cont[] = {0.0,0.0,0.0,0.0}; // front left up yaw
    ...
    while(is_run_thread){ // 쓰레드 종료할 수 있게 코드 추가하기 !!
      ...
      if(chk_x != x_tf){
        if(x_tf > 0.05){ // 여기다가 이동하는 코드 넣기.
          ...
          float cont[] = {0.0,-0.1,0.0,0.0}; //전좌상회전
          move(cont,0.2);
      ...
      if(chk_y != y_tf){
        if(y_tf > 0.05){
          float cont[] = {-0.1,0.0,0.0,0.0}; //전좌상회전
          move(cont,0.2);
      ...
      // cout << "LR : " << x_tf << " FB : " << y_tf << endl;
      // cout << chk_x << " " << chk_y << endl;
      // cout << typeid(x_tf).name() << endl;
      // cout << typeid(chk_x).name() << endl;
      // cout << endl;
  ...
  auto control_gimbal = []()
    ...
    */
    ...
    while(is_run_thread){
      ...
      break; // 이거는 짐벌 제어할 필요 없을거 같아서 그냥 종료
    }
	};
