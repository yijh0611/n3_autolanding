...
// Platform 위치
void cb_target(const std_msgs::Float64MultiArray target);
...
int main(int argc, char **argv){
  ...
  vo_x,vo_y,v_z,\
  target_x, target_y,vel_platform_r_abs,vel_platform_f_abs,\
  move_right,dist_r,vel_platform_r,move_r_real,vel_r_drone,\
  theta,theta_deg,vo_x_cal";
  ...
  ros::Subscriber sub_target = nh.subscribe("platform_position", 1000, cb_target);
  ...
  auto save_log = []()
    ...
    ros::Rate loop_rate_save(100);
      ...
      outfile << target_x << "," << target_y << "," << vel_platform_r_abs << ",";
      outfile << vel_platform_f_abs << "," << move_right << "," << dist_r << ",";
      outfile << vel_platform_r << "," << move_right_real << "," << vel_r_drone << ",";
      outfile << theta << "," << theta_deg << "," << vo_x_cal << endl;
...
void cb_target(const std_msgs::Float64MultiArray target){
  ...
  vel_platform_r_abs = target.data[2];
  vel_platform_f_abs = target.data[3];
  move_right = target.data[4];
  dist_r = target.data[5];
  vel_platform_r = target.data[6];
  move_right_real = target.data[7];
  vel_r_drone = target.data[8];
  theta = target.data[9];
  theta_deg = target.data[10];
  vo_x_cal = target.data[11];
}