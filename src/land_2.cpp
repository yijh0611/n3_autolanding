...
// is_tag_signal은 무조건 true여야 되는거 아닌가?
  ...
  auto control_drone = []()
    ...
    while(is_run_thread){ // 이게 수정된 제어 코드
      ...
      if(tag_x_tmp != x_tf){ // 태그 정보가 바뀌었을때
        ...
        if (is_boost){ // 이거는 안쓸거라서 주석해도 될 듯
        ...
        float spd_constant = 0; // 0.85 // 사용 안할거라서 0
        ...
        else if (tf_name == "tag_3"){
          target_height = 1;
        }
        else if (tf_name == "tag_4"){
          target_height = 0;
          if (fabs(z_tf) < 0.3){
            is_land = true;
          }
        ...
        if(fabs(z_tf - target_height) > 0.05){
          move_up = (target_height - z_tf) * 0.3;
          // cout << "Height : " << z_tf << endl;
        }
...
{ // 값 저장하는 코드
  if (is_tag_signal){
    string name_tmp = msg3.transforms[0].child_frame_id;
    // cout << name_tmp << endl;
    // 작은 태그일 때
    if(name_tmp == "tag_4"){ // 6
      // 작은 태그가 한번 보이면 큰 태그 우선순위 지우기 위함
