...
runWaypointMission(uint8_t numWaypoints, int responseTimeout)
{
  ...
  // Waypoint Mission : Initialization // 이거 꼭 해야 되는건가?
...
obtainCtrlAuthority()
{
  ...
  sdkAuthority.request.control_enable = 1; // 권한 요청하기 위해 1(True)라고 하는 듯?
  sdk_ctrl_authority_service.call(sdkAuthority); // 권한 요청?
...
main(int argc, char** argv)
{
  ...
  ServiceAck ack = obtainCtrlAuthority(); // 이거 꼭 해야지 조종이 가능해지는건가? - 이 권한을 받으면, RC로 조종이 안되나?
  if (ack.result)
  {
  ...
  // Custom code
  ros::Publisher control_vel_pub = nh.advertise<sensor_msgs::Joy>("dji_sdk/flight_control_setpoint_ENUvelocity_yawrate", 10);
  
  ros::Rate loop_rate(10);
  int count = 0;
  float input_drone = 80.0;
  ...
  switch (inputChar)
  {
    ...
    case 'c': // custom
      while (ros::ok())
        {    
          if(true) // takeoff_result //takeoff().result
          { // 여기에 코드 추가
            if(input_drone < 100){
              sensor_msgs::Joy msg_control;
              msg_control.axes = {input_drone,input_drone,0.0,0.0};
              //msg_control.buttons = {1,0,0,0};
              msg_control.header.seq = 1;
              msg_control.header.stamp = ros::Time::now();
              msg_control.header.frame_id = "0";
              //msg_control.buttons = {};
              std::cout << msg_control << std::endl;
              input_drone = input_drone + 5.0;
              control_vel_pub.publish(msg_control); // 이거 수정하면 다른 명령 줄 수 있음.
