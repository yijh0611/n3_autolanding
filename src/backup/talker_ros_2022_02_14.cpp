...
/*void cb::callback(const sensor_msgs::Imu::ConstPtr &msg)
{
  ...
  while (ros::ok())
  {
    ...
    if(takeoff_result)
    {
      ...
      if(ros::ok()){
        ...
        //msg_control.buttons = {};
        //std::cout << msg_control_gen << std::endl;
        ...
        */
        ...
        msg_control.axes = {70.0, 0.0,0.0,0.0};
        std::cout << msg_control << std::endl;
        control_vel_pub.publish(msg_control);
        ros::Duration(0.1).sleep();
        */
        //input_drone = 20 - input_drone;
        ...
        msg_control.axes = {chk - input_drone, input_drone,0.0,0.0}; // 이거만 있으면 드론 움직임.  , {0.0,80.0,0.0,0.0}
        control_vel_pub.publish(msg_control); // 이거 수정하면 다른 명령 줄 수 있음.
        */
