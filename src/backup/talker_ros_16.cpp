...
int main(int argc, char **argv){
  ...
  auto control_drone = []()
    ...
    while(is_run_thread){ // 이게 수정된 제어 코드
      // if(kal_time_tmp != kal_time){ // 태그 정보가 바뀌었을때
      //   if(kal_is_tag > 0 && kal_is_tag_lost == 0){ // 태그가 보일 때
      //     if(r_prev == 0){
      //       r_prev = kal_r;
      //       f_prev = kal_f;
      //     }
      //     float d_r = kal_r - r_prev;
      //     float d_f = kal_f - f_prev;

      //     float dt = kal_time - kal_time_tmp;
      //     float vr_tag = d_r / dt;
      //     float vf_tag = d_f / dt;
      //     if(dt < 0){ // 오버플로우 있을 때 - 한번만 잘 넘기면 된다. (9999/60분에 한번 발생한다.)
      //       vr_tag = 0;
      //       vf_tag = 0;
      //     }

      //     float right_tag = kp * kal_r + kd * vr_tag; //+ ki * 0;
      //     float front_tag = kp * kal_f + kd * vf_tag;

      //     move_right = kal_r_vel + right_tag;
      //     move_front = kal_f_vel + front_tag;
      ...
      //     kal_time_tmp = kal_time;
      //   }

      //   control_time = ros::Time::now();
      //   continue;
      // }
      // else if(kal_is_tag > 0){
      //   if(ros::Time::now() - control_time > ros::Duration(time_limit)){
      //     move_front = 0.0;
      //     move_left = 0.0;
      //     move_right = 0.0;
      //     float cont[] = {move_front,move_left,0.0,0.0}; //전좌상회전
